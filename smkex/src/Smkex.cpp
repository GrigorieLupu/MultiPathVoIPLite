    ...
    const uint8_t *msg, uint32_t msgLen, int channel)
{
  ...
  case ratchetDHKey:
    MP_LOG1("received ratchetDHKey\n");

    if (SMKEX_CHANNEL_1 == channel)
    {
      SmkexSessionInfo &session = getSessionInfo(buddy);

      if (!session.isVerticalRatchetInitialized())
      {
        MP_LOG1("Vertical ratchet not initialized, ignoring DH key\n");
        break;
      }

      if (SMKEX_PUB_KEY_LEN < rec.getData(NULL))
      {
        MP_LOG1("Not enough memory for DH ratchet key\n");
        return -1;
      }

      // Extrage noua cheie publică DH
      unsigned char new_dh_pub_key[SMKEX_PUB_KEY_LEN];
      int key_length = rec.getData(new_dh_pub_key);

      printf("=== RECEIVED RATCHET DH KEY ===\n");
      printf("Received DH ratchet key with length %d\n", key_length);
      printf("DH key (first 16 bytes): ");
      for(int i = 0; i < 16 && i < key_length; i++)
          printf("%02X", new_dh_pub_key[i]);
      printf("...\n");

      // Afișează starea înainte de vertical ratchet
      printf("State before vertical ratchet step:\n");
      session.debugVerticalRatchetState();

      // Efectuează pasul de vertical ratchet
      if (!session.performVerticalRatchetStep(new_dh_pub_key, key_length))
      {
        MP_LOG1("Error performing vertical ratchet step\n");
        return -1;
      }

      MP_LOG1("Vertical ratchet step completed successfully\n");

      // Afișează starea după vertical ratchet
      printf("State after vertical ratchet step:\n");
      session.debugVerticalRatchetState();
      printf("==============================\n\n");

      // Răspunde cu propria noastră cheie publică DH
      int result = sendRatchetDHKey(session, buddy);
      if (result == 0) {
          MP_LOG1("Sent response DH ratchet key\n");
      } else {
          MP_LOG1("Error sending response DH ratchet key\n");
      }
    }
    break;
...
// Symmetrical Ratcheting
// Adaugă metodă pentru trimiterea cheilor DH de ratchet
int Smkex::sendRatchetDHKey(SmkexSessionInfo &session, std::string buddy) {
    if (!session.isVerticalRatchetInitialized()) {
        MP_LOG1("Error: Vertical ratchet not initialized\n");
        return -1;
    }
    ...
    if (key_length == 0) {
        MP_LOG1("Error: No local DH ratchet key available\n");
        return -1;
    }
    
    MP_LOG1("Sending DH ratchet key to buddy\n");
    SmkexT4mRecord sr(0, buddy, getClientID2(), ratchetDHKey,
                      SMKEX_T4M_PROTOCOL_VERSION, key_length, local_dh_ratchet_key);
