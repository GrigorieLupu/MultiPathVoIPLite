    ...
    const uint8_t *msg, uint32_t msgLen, int channel)
{
  ...
  case handshakeNonceH:
    ...
    if (SMKEX_CHANNEL_1 == channel && session.getState() == SmkexState::STATEWaitNonceH)
    {
      ...
      // VERTICAL RATCHETING - Initialize vertical ratchet tracking
      printf("=== VERTICAL RATCHET SETUP ===\n");
      printf("Session now connected - vertical ratcheting enabled\n");
      printf("Vertical ratchet interval: %d messages\n", VERTICAL_RATCHET_INTERVAL);
  ...
  case verticalRatchetInitiate:
    MP_LOG1("received verticalRatchetInitiate\n");
    
    if(SMKEX_CHANNEL_0 == channel && session.getState() == SmkexState::STATEConnected) {
      printf("=== PROCESSING VERTICAL RATCHET INITIATE ===\n");
      printf("From buddy: %s\n", buddy.c_str());
      printf("Current session state before vertical ratchet:\n");
      session.printVerticalRatchetState();
      session.printRatchetState();
      
      unsigned char* data_ptr = new unsigned char[rec.getData(NULL)];
      uint32_t data_len = rec.getData(data_ptr);
      
      int result = processVerticalRatchetInitiate(session, data_ptr, data_len);
      
      if(result == 0) {
        printf("Vertical ratchet initiate processed successfully\n");
        printf("New session state after vertical ratchet:\n");
        session.printVerticalRatchetState();
        session.printRatchetState();
      } else {
        printf("Error processing vertical ratchet initiate\n");
      }
      ...
      MP_LOG1("Error: vertical ratchet initiate in wrong state or channel\n");
      return -1;
    }
    break;
  ...
  case verticalRatchetResponse:
    ...
    if(SMKEX_CHANNEL_0 == channel && session.getState() == SmkexState::STATEConnected) {
      ...
      printf("From buddy: %s\n", buddy.c_str());
      printf("Current session state before finalizing vertical ratchet:\n");
      session.printVerticalRatchetState();
      session.printRatchetState();
      ...
      int result = processVerticalRatchetResponse(session, data_ptr, data_len);
      
      if(result == 0) {
        printf("Vertical ratchet response processed successfully\n");
        printf("Final session state after vertical ratchet completion:\n");
        session.printVerticalRatchetState();
        session.printRatchetState();
      } else {
        printf("Error processing vertical ratchet response\n");
      }
      ...
      MP_LOG1("Error: vertical ratchet response in wrong state or channel\n");
      return -1;
    }
    break;
...
int Smkex::checkAndInitiateVerticalRatchet(SmkexSessionInfo &session, std::string buddy)
{
  if (!session.isRatchetInitialized())
  {
    return 0; // Nu facem vertical ratchet până nu e inițializat symmetric ratchet
  }

  if (session.shouldPerformVerticalRatchet())
  {
    MP_LOG1("Initiating vertical ratchet due to counter threshold\n");
    printf("=== AUTO VERTICAL RATCHET TRIGGER ===\n");
    printf("Buddy: %s\n", buddy.c_str());
    printf("Sending counter: %u\n", session.getSendingCounter());
    printf("====================================\n");

    if (session.initiateVerticalRatchet() != 0)
    {
      MP_LOG1("Error initiating vertical ratchet\n");
      return -1;
    }

    return sendVerticalRatchetInitiate(session, buddy);
  }

  return 0;
}

int Smkex::sendVerticalRatchetInitiate(SmkexSessionInfo &session, std::string buddy)
{
  MP_LOG1("Sending vertical ratchet initiate message\n");

  // Obține cheia publică DH temporară generată
  unsigned char temp_pub_key[SMKEX_PUB_KEY_LEN];
  int key_len = session.getLocalPubKey(temp_pub_key);

  if (key_len != SMKEX_PUB_KEY_LEN)
  {
    MP_LOG1("Error getting local public key for vertical ratchet\n");
    return -1;
  }

  printf("VERTICAL_RATCHET_SEND: Sending initiate message to %s\n", buddy.c_str());
  printf("Public key (first 8 bytes): ");
  for (int i = 0; i < 8; i++)
    printf("%02X", temp_pub_key[i]);
  printf("...\n");

  SmkexT4mRecord rec(0, buddy, getClientID(), verticalRatchetInitiate,
                     SMKEX_T4M_PROTOCOL_VERSION, key_len, temp_pub_key);

  rec.printRecord();
  return sendRecord(rec, buddy, SMKEX_CHANNEL_0);
}

int Smkex::sendVerticalRatchetResponse(SmkexSessionInfo &session, std::string buddy)
{
  MP_LOG1("Sending vertical ratchet response message\n");

  // Obține cheia publică DH generată ca răspuns
  unsigned char temp_pub_key[SMKEX_PUB_KEY_LEN];
  int key_len = session.getLocalPubKey(temp_pub_key);

  if (key_len != SMKEX_PUB_KEY_LEN)
  {
    MP_LOG1("Error getting local public key for vertical ratchet response\n");
    return -1;
  }

  printf("VERTICAL_RATCHET_SEND: Sending response message to %s\n", buddy.c_str());
  printf("Public key (first 8 bytes): ");
  for (int i = 0; i < 8; i++)
    printf("%02X", temp_pub_key[i]);
  printf("...\n");

  SmkexT4mRecord rec(0, buddy, getClientID(), verticalRatchetResponse,
                     SMKEX_T4M_PROTOCOL_VERSION, key_len, temp_pub_key);
...
int Smkex::processVerticalRatchetInitiate(SmkexSessionInfo &session, const unsigned char *data, uint32_t dataLen)
{
  MP_LOG1("Processing vertical ratchet initiate message\n");

  if (dataLen != SMKEX_PUB_KEY_LEN)
  {
    MP_LOG1("Error: Invalid key length in vertical ratchet initiate\n");
    return -1;
  }

  printf("VERTICAL_RATCHET_RECV: Processing initiate message\n");
  printf("Received key (first 8 bytes): ");
  for (int i = 0; i < 8; i++)
    printf("%02X", data[i]);
  printf("...\n");

  // Procesează cheia primită și generează răspuns
  if (session.processVerticalRatchetKey(data, dataLen) != 0)
  {
    MP_LOG1("Error processing vertical ratchet key\n");
    return -1;
  }

  // Trimite răspuns cu cheia noastră
  return sendVerticalRatchetResponse(session, session.getBuddy());
}

int Smkex::processVerticalRatchetResponse(SmkexSessionInfo &session, const unsigned char *data, uint32_t dataLen)
{
  MP_LOG1("Processing vertical ratchet response message\n");

  if (dataLen != SMKEX_PUB_KEY_LEN)
  {
    MP_LOG1("Error: Invalid key length in vertical ratchet response\n");
    return -1;
  }

  if (!session.isAwaitingVerticalRatchetResponse())
  {
    MP_LOG1("Warning: Received vertical ratchet response but not awaiting one\n");
    return -1;
  }

  printf("VERTICAL_RATCHET_RECV: Processing response message\n");
  printf("Received key (first 8 bytes): ");
  for (int i = 0; i < 8; i++)
    printf("%02X", data[i]);
  printf("...\n");

  // Finalizează vertical ratchet cu cheia primită
  if (session.processVerticalRatchetKey(data, dataLen) != 0)
  {
    MP_LOG1("Error processing vertical ratchet response key\n");
    return -1;
  }
