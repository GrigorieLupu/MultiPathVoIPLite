...
void print_usage(const std::string &buddy) {
  ...
  std::cout << "v - Show vertical ratchet state" << std::endl;
  std::cout << "f - Force vertical ratchet" << std::endl;
  std::cout << "p - Process WebSocket messages" << std::endl; // NEW
  std::cout << "++++++++++++++++++++++++++++++++++++++++++++++++" << std::endl;
}
...
int main(int argc, char *argv[])
{
  ...
  while (true)
  {
    ...
    case 'v':
  {
      /* Show vertical ratchet state */
      std::cout << std::endl << "Action: Show vertical ratchet state!" << std::endl;
      SmkexSessionInfo &session = smkex.getSessionInfo(buddyID);
      session.printVerticalRatchetState();
      
      // Procesează mesajele WebSocket după afișare
      std::cout << "Checking for new WebSocket messages..." << std::endl;
    ...
    case 'f':
    {
      ...
      {
        std::cout << "Error: Ratchet not initialized!" << std::endl;
        break;
      }

      std::cout << "Forcing vertical ratchet..." << std::endl;

      if (session.initiateVerticalRatchet() == 0)
      {
        if (smkex.sendVerticalRatchetInitiate(session, buddyID) == 0)
        {
          std::cout << "Vertical ratchet initiated successfully!" << std::endl;
        }
        else
        {
          std::cout << "Error sending vertical ratchet initiate!" << std::endl;
        }
      }
      else
      {
        std::cout << "Error initiating vertical ratchet!" << std::endl;
      }
      break;
    }
